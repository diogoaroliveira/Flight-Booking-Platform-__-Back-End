...
elemento *elemento_novo(char *prioridade, no_grafo *no)
{
    if (!no)
        return NULL;
    ...
    elem->prioridade = (char*) realloc(elem->prioridade, (strlen(prioridade)+1)*sizeof(char));
    strcpy(elem->prioridade, prioridade);

    return elem;
}

int heap_insere(heap *h, no_grafo *no, char *prioridade)
{
...
int menor_que(elemento *e1, elemento *e2)
{
    ...
    if(strcmp(e1->prioridade, e2->prioridade) < 0) return 1;
